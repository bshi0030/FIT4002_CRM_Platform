...
export default function Dashboard() {
  ...
  // Generate mock KPI data locally instead of fetching
  const generateMockData = useCallback(() => {
    ...
    const baseSales = 625000;
    const totalSales = baseSales + rand(-3000, 5000);
    const avgDeal = Math.round(totalSales / completedDeals);

    const newData = {
      totalSales: {
        value: totalSales,
        changePercent: parseFloat((rand(108, 130) / 10).toFixed(1)),
        vsLastMonth: Math.round(totalSales * 0.89)
      },
      dealsCompleted: {
        value: completedDeals,
        changePercent: parseFloat((rand(55, 95) / 10).toFixed(1)),
        vsLastMonth: completedDeals - rand(20, 30)
      },
      ongoingDeals: {
        value: ongoingDeals,
        changePercent: parseFloat((rand(30, 60) / 10).toFixed(1)),
        vsLastMonth: ongoingDeals - rand(5, 10)
      },
      avgDealValue: {
        value: avgDeal,
        changePercent: parseFloat((rand(140, 165) / 10).toFixed(1)),
        vsLastMonth: avgDeal - rand(200, 600)
      },
      pipeline: {
        stages: [
      ...
      salesTrends: [
        { week: 'Week 1', sales: rand(80000, 100000), deals: rand(40, 55) },
        { week: 'Week 2', sales: rand(95000, 120000), deals: rand(48, 62) },
        { week: 'Week 3', sales: rand(105000, 135000), deals: rand(55, 70) },
        { week: 'Week 4', sales: rand(130000, 160000), deals: rand(65, 80) }
      ],
      activitySummary: {
        callsMade:     rand(40, 50),
        meetingsHeld:  rand(15, 22),
        emailsSent:    rand(120, 135),
        dealsClosed:   rand(10, 15),
        callsChange:   12,
        meetingsChange: 12,
        emailsChange:  -9,
        dealsChange:   12
  ...
  const pieData  = pipeline
    ...
    : [];
  const pct = pipeline
    ? Math.round((pipeline.completedDeals / pipeline.total) * 100)
    : 0;
    ...
    <div className="dashboard">
      ...
      <div className="live-banner">
        ...
        <span className="live-text">
          Live &middot; Updated {lastUpdated || clock}
        </span>
        ...
        <div className="chart-card pipeline-card">
          <div className="card-header">
            <div>
              ...
              <p className="card-sub">Completed vs ongoing deals</p>
            </div>
            {pipeline && (
              <span className="total-badge">{pipeline.total} Total</span>
            )}
          </div>
          ...
          <div className="pipeline-body">
            ...
            <div className="donut-wrap">
              ...
              <div className="donut-legend">
                ...
                <strong>{pipeline?.completedDeals ?? '—'}</strong>
                <span className="dot" style={{ background: CHART_GREY }} />
                <span>Ongoing Deals</span>
                <strong>{pipeline?.ongoingDeals ?? '—'}</strong>
              </div>
            </div>
        ...
        <div className="chart-card activity-card">
          <div className="card-header">
            <div>
              ...
              <p className="card-sub">All team activities</p>
            </div>
          </div>
              ...
              return (
                <div key={key} className="activity-tile">
                  <div className="activity-tile-top">
                    ...
                    <span className={`kpi-badge ${pos ? 'badge-up' : 'badge-down'}`} style={{ fontSize: 11 }}>
                      {pos ? '+' : ''}{chg}%
                    </span>
                  </div>
            ...
            <div className="recent-activities-list">
              {recentActivities.map(act => (
                <div key={act.id} className="recent-activity-item" style={{ backgroundColor: act.id === 1 ? '#F7E99A' : '#fff' }}>
                  <div className="recent-activity-icon" style={{ backgroundColor: act.bg, color: act.color }}>
                    {act.icon}
                  </div>
        ...
        <div className="chart-card team-card">
          <div className="card-header">
            <div>
              ...
              <p className="card-sub">Compare team member metrics</p>
            </div>
            {topMember && (
              <span className="top-badge"><FiAward size={13} style={{ marginRight: 4, verticalAlign: 'middle' }} />Top: {topMember}</span>
            )}
          </div>
          ...
          <table className="team-table">
            ...
            <tbody>
              {loading
                ? [1,2,3].map(i => (
                    <tr key={i}>
                      {[1,2,3,4].map(j => <td key={j}><Skeleton h={12} /></td>)}
                    </tr>
                  ))
                : teamMembers.map((m) => (
          ...
          {!loading && (
            <div className="team-footer">
              <div className="team-footer-item">
                <span className="tf-icon" style={{ background: ACCENT }}><FiBarChart2 size={16} /></span>
                <span>Total Sales</span>
                <strong>{fmtMoney(teamTotal)}</strong>
              </div>
              <div className="team-footer-item">
                <span className="tf-icon" style={{ background: CHART_GREEN }}><FiGitMerge size={16} /></span>
                <span>Total Deals</span>
                <strong>{teamDeals}</strong>
              </div>
              <div className="team-footer-item">
                <span className="tf-icon" style={{ background: '#aaa' }}><FiActivity size={16} /></span>
                <span>Avg per Member</span>
                <strong>{fmtMoney(teamAvg)}</strong>
              </div>
            </div>
          )}
        </div>
